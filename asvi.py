...
class Inference_RNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, rnn_type='RNN'):
        super(Inference_RNN, self).__init__()

        rnn_cls = {'RNN': nn.RNN, 'GRU': nn.GRU, 'LSTM': nn.LSTM}[rnn_type]
        self.rnn = rnn_cls(input_dim, hidden_dim, num_layers=2, batch_first=True)
    ...
    def forward(self, x):
        ...
        h_out, _ = self.rnn(x)  # h_out: (batch_size, seq_len, hidden_dim)

        # Apply the same output head to all time steps
        mean = self.FC_mean(h_out).squeeze(-1)       # (batch_size, seq_len)
        sd_log = self.FC_sd_log(h_out).squeeze(-1)   # (batch_size, seq_len)
...
class Model(nn.Module):
    ...
    def variational_z(self, x):
        ...
        elif (self.use_asvi and self.input_dim == 1):
            a_i, b_i, nu_sd_z_log = self.inference_nn(x.reshape(N, 1))
            b_i = b_i[:-1]
            nu_sd_z = torch.exp(nu_sd_z_log)
        elif (self.use_asvi and self.input_dim == 2):
            x1 = torch.cat((torch.tensor([0]), x[:-1]),dim = 0)
            x_input = torch.transpose(torch.stack([x1, x]), 0, 1)
            a_i, b_i, nu_sd_z_log = self.inference_nn(x_input)
            b_i = b_i[:-1]
            nu_sd_z = torch.exp(nu_sd_z_log)
        elif (self.use_asvi and self.input_dim == 3):
            x1 = torch.cat((torch.tensor([0]), x[:-1]),dim = 0)
            x2 = torch.cat((torch.tensor([0]), x1[:-1]),dim = 0)
            x_input = torch.transpose(torch.stack([x2, x1, x]), 0, 1)
            a_i, b_i, nu_sd_z_log = self.inference_nn(x_input)
...
methods = {
    ...
    "ASVI, k=1": {"use_asvi": True, "const_z": False, "input_dim": 1, "hidden_dim": 20, "alpha": 0.99},
    "ASVI, k=2": {"use_asvi": True, "const_z": False, "input_dim": 2, "hidden_dim": 20, "alpha": 0.99},
    "ASVI, k=3": {"use_asvi": True, "const_z": False, "input_dim": 3, "hidden_dim": 20, "alpha": 0.99}}
...
for N in sample_sizes:
    ...
    for i in range(N - 1):
        z[i + 1] = 0.5*z[i] + torch.normal(mean=torch.tensor(0.0), std=tau)
...
np.savez(
    os.path.join(output_dir, "ASVI_experiment_results.npz"),
